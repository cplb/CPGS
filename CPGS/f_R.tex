...
\label{eq:Uniform}
\end{equation}
where we have assumed that $r > L$ at all stages.\footnote{Inside the source $R^{(1)} = -{8 G \Upsilon^2 M}/{c^2}\left[1 - (\Upsilon a + 1)\exp(-\Upsilon a)\sinh(\Upsilon r)/\Upsilon r\right]$.}
...
\begin{equation}
U(r) = \frac{GM}{^2r}\left[1 + \frac{\exp(- \Upsilon r)}{3}\right].
\end{equation}
